...
export default class SpotifyPlayCommand extends Command {
    ...
    public async exec(message: Message, { search }) {
        return spotify.search({ type: "track", query: `${search}` })
            .then(async (response) => {
                ...
                let track = response.tracks.items.slice(0, 5);

                if (response.tracks.items.length < track)
                    track = response.tracks.length;
                ...
                const collector = message.channel.createMessageCollector(m => m.author.id === message.author.id && new RegExp(`^([1-5]|cancel)$`, "i").test(m.content), { time: 30e3, max: 1 });

                collector.on("collect", async (m) => {
                    if (/cancel/i.test(m.content))
                        return collector.stop("cancelled");

                    const trc = track[Number(m.content) - 1];
                    let res;
                    try {
