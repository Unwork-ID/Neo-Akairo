...
export default class KurumiWallpaper extends Command {
    public constructor() {
        super("kruumi", {
            aliases: ["kurumi"],
            category: "Wallpaper",
            description: {
                content: "Give you random astolfo wallpaper from < tokisaki.xyz/api/v1/astolfo >",
                usage: "kurumi",
                example: [
                    "kurumi"
                ]
    ...
    public async exec(message: Message): Promise<Message> {
            ...
            let res = await api.KurumiRandom()
