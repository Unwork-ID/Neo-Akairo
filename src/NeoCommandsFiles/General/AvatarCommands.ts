...
import { Message, MessageEmbed, GuildMember, ImageSize, AllowedImageFormat } from 'discord.js';
...
export default class AvatarCommand extends Command {
    ...
    public exec(message: Message, { member, size, format}: { member: GuildMember, size: number, format: string}): Promise<Message> {
        ...
        .setURL(member.user.displayAvatarURL({ format: format as AllowedImageFormat, size:size as ImageSize}))
        .setColor("RANDOM")
        .setImage(member.user.displayAvatarURL({ format: format as AllowedImageFormat, size:size as ImageSize}))
        );
    };
};